(function () {
  ...
  function installBrowserPreviewApi() {
    ...
    const mockChallenges = [
      { id: "preview-signin", friendlyId: "signin", groundId: "BUUCTF", groundName: "BUUCTF", name: "签到", category: "MISC", difficulty: "Easy", description: "签到题 flag{buu_ctf}", points: 1, solveCount: 827, files: [{ id: "readme", name: "readme.txt", size: 512, type: "text" }] },
      { id: "preview-warmup", friendlyId: "warmup", groundId: "BUUCTF", groundName: "BUUCTF", name: "[HCTF 2018] WarmUp", category: "WEB", difficulty: "Easy", description: "Web 入门题。", points: 1, solveCount: 467, files: [] },
    ];
    ...
    window.ctfCompass = {
      ...
      checkForUpdates: () => delay({ currentVersion: "0.9.2", latestVersion: "0.9.2", hasUpdate: false, name: "CTF Compass v0.9.2", url: "https://github.com/Becauseiloveyo/ctf-compass1/releases/latest", assetCount: 1 }),
  ...
  function installUpdateCard() {
    ...
    row.innerHTML = `
      <div>
        <strong>版本更新</strong>
        <p id="update-status-text">当前版本随 Release 发布，可手动检查新版。</p>
      </div>
      <div class="settings-actions">
        <button id="check-update-button" class="secondary-button" type="button">检查更新</button>
        <button id="open-release-button" class="secondary-button" type="button">打开 Release</button>
      </div>
    `;
    list.append(row);
    ...
    let releaseUrl = "https://github.com/Becauseiloveyo/ctf-compass1/releases/latest";

    async function check() {
      if (!window.ctfCompass.checkForUpdates) return;
      ...
      try {
        ...
        status.textContent = result.hasUpdate
          ? `发现新版 ${result.name || result.latestVersion}，当前为 v${result.currentVersion}。`
  ...
  function installCtf2ConnectorUi() {
    ...
    const state = { status: null, challenges: [], selected: null, busy: false, searchTimer: null, diagnosticRows: [] };
    ...
    view.innerHTML = `
      <div class="ctf2-toolbar panel">
        <div><p class="panel-kicker">CTF2 CONNECTOR</p><h3 class="panel-title">直接浏览并导入 CTF2 题目</h3><p class="body-copy">题库可公开浏览；附件下载需要登录令牌。</p></div>
        <div class="ctf2-account-actions"><span id="ctf2-account-status" class="scope-badge">检查登录状态...</span><button id="ctf2-diagnose-button" class="secondary-button" type="button">诊断</button><button id="ctf2-copy-diagnostic-button" class="secondary-button" type="button" disabled>复制日志</button><button id="ctf2-login-button" class="secondary-button" type="button">应用内登录</button><button id="ctf2-system-login-button" class="secondary-button" type="button">浏览器登录</button><button id="ctf2-logout-button" class="secondary-button danger-button" type="button">退出</button></div>
      </div>

      <details class="panel ctf2-token-help">
        <summary>浏览器 token</summary>
        <div class="ctf2-token-help-body"><p class="body-copy">Passkey 无法在应用内验证时，使用系统浏览器登录后复制 <code>localStorage.getItem("token")</code> 的值。token 只加密保存在本机。</p><div class="ctf2-token-row"><label class="field ctf2-token-field" for="ctf2-token-input"><span>粘贴 token</span><input id="ctf2-token-input" type="password" placeholder="在这里粘贴 CTF2 token" autocomplete="off" /></label><button id="ctf2-token-import-button" class="primary-button" type="button">验证并连接</button></div><div id="ctf2-diagnostic-list" class="stack-list compact-stack" hidden></div></div>
      </details>

      <div class="ctf2-layout">
        <section class="panel ctf2-browser-panel"><div class="ctf2-filters"><label class="field ctf2-search-field"><span>搜索题目</span><input id="ctf2-search-input" type="search" placeholder="题目名、描述或编号" autocomplete="off" /></label><label class="field"><span>题型</span><select id="ctf2-category-select"><option value="all">全部</option></select></label><button id="ctf2-refresh-button" class="secondary-button" type="button">刷新</button></div><div class="section-label-row"><div><h3 class="section-title">BUUCTF 公开练习题</h3><p id="ctf2-result-count" class="body-copy">尚未加载。</p></div></div><div id="ctf2-challenge-list" class="ctf2-challenge-list"></div></section>
        <aside class="panel ctf2-detail-panel"><div class="panel-head compact-head"><div><p class="panel-kicker">SELECTED CHALLENGE</p><h3 id="ctf2-detail-title" class="panel-title">选择一道题目</h3></div></div><div id="ctf2-detail-meta" class="ctf2-detail-meta"></div><p id="ctf2-detail-description" class="body-copy">选择题目后可查看描述、附件和导入操作。</p><div id="ctf2-file-list" class="stack-list compact-stack"></div><button id="ctf2-import-button" class="primary-button ctf2-import-button" type="button" disabled>导入并求解</button><p id="ctf2-import-status" class="empty-copy">不会自动启动靶机，也不会自动提交 flag。</p></aside>
      </div>
    `;
    main.append(view);
    ...
    const el = {
      ...
      copyDiagnostic: view.querySelector("#ctf2-copy-diagnostic-button"),
      ...
      importStatus: view.querySelector("#ctf2-import-status"),
    };
    ...
    function setBusy(busy) {
      state.busy = busy;
      [el.login, el.systemLogin, el.logout, el.importToken, el.refresh, el.importChallenge, el.diagnose, el.copyDiagnostic].forEach((button) => {
        if (!button) return;
        button.disabled = busy || (button === el.logout && !state.status?.connected) || (button === el.importChallenge && !state.selected?.files?.length) || (button === el.copyDiagnostic && !state.diagnosticRows.length);
      });
    }
    ...
    function activateCtf2View() {
      ...
      if (title) title.textContent = "题库连接器";
    ...
    function renderDetail() {
      ...
      if (selected) {
        [selected.category, selected.difficulty, selected.points ? `${selected.points} 分` : "", selected.solveCount ? `${selected.solveCount} solves` : ""].filter(Boolean).forEach((label) => {
    ...
    function diagnosticText() {
      const selected = state.selected ? `${state.selected.name} (${state.selected.id})` : "未选择";
      return [
        "CTF Compass CTF2 Diagnostic",
        `Time: ${new Date().toISOString()}`,
        `Selected: ${selected}`,
        ...state.diagnosticRows.map((row) => `${row.title}: ${row.note}`),
      ].join("\n");
    }
    ...
    async function diagnoseConnection() {
      ...
      try {
        ...
        state.status = status;
        rows.push({ title: status?.connected ? "登录状态正常" : "未登录或 token 已失效", note: status?.connected ? `已识别账号${status.profile?.username ? `：${status.profile.username}` : ""}` : status?.error || "题库可公开浏览；下载附件需要应用内登录或浏览器 token。" });
        renderStatus();
      } catch (error) {
    ...
    async function importChallenge() {
      ...
      try {
        ...
        if (typeof window.runAnalysis === "function" && (imported.artifacts || []).length) {
          window.setTimeout(() => window.runAnalysis({ focusResults: true, doneMessage: "CTF2 附件已导入并完成自动求解。" }), 0);
    ...
    el.copyDiagnostic.addEventListener("click", async () => {
      const ok = await copyText(diagnosticText());
      setStatus(ok ? "诊断日志已复制。" : "复制失败，请手动截图诊断内容。", ok ? "info" : "error");
    });
